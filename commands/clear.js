...
module.exports = (client, message, arguments) => {
    ...
    if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("Disculpe, pero usted no tiene los poderes para hacer eso.");
    if(!arguments[0]) return message.reply("Disculpe, necesito un numero para trabajar.");

    if(arguments[0]>100){
        message.channel.send("Disculpe, pero solo puedo limpiar 100 mensajes a la vez")
        .then((msg) => {
            msg.delete(5000);
        })
        .cath((error) => {
